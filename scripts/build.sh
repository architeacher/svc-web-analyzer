...
build_binaries() {
    ...
    while IFS= read -r -d '' go_file; do
        ...
        build_command+=" go build -v -o ${binary_name}"
        build_command+=" -installsuffix \"${target_os}.${target_arch}\""
        build_command+=" -asmflags='-trimpath=${package_dir}'"
        build_command+=" -gcflags \"${gc_flags}\""
        build_command+=" -ldflags \"${linker_flags}\""
        build_command+=" -tags ${go_tags}"
        build_command+=" -trimpath ${go_file}"
...
Options:
  -s, --src-dir       Source directory containing Go files
  -o, --output-dir    Output directory for the binaries
  -os, --os           Target operating system (e.g., darwin, linux, windows)
  -a, --arch          Target architecture (e.g., 386, amd64, arm, arm64)
  -l, --log-level     Log level for messages (debug, info, notice, warning, error, critical, alert, emergency)
  -d, --debug         Enable debugging flags
  -h, --help          Display this help information
  -x                  Display this execution flow
