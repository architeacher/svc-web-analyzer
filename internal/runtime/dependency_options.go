...
func WithHTTPServer() DependencyOption {
	return func(d *Dependencies) error {
		...
		analysisService := service.NewApplicationService(
			repos.NewAnalysisRepository(db),
			repos.NewOutboxRepository(db),
			repos.NewCacheRepository(
				d.Infra.CacheClient,
				d.cfg.Cache,
				d.logger,
			),
...
func WithSubscriber() DependencyOption {
	return func(d *Dependencies) error {
		...
		if d.Infra.CacheClient != nil {
			cacheRepo = repos.NewCacheRepository(d.Infra.CacheClient, d.cfg.Cache, d.logger)
		}
