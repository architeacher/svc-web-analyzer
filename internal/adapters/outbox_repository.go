...
func (r *OutboxRepository) SaveInTx(tx *sqlx.Tx, event *domain.OutboxEvent) error {
...
// MarkCompleted marks when Subscriber completes the analysis (triggers duration calculation).
...
// unmarshalPayload unmarshals the payload JSON into the appropriate type based on an event type.
...
// CalculateBackoffDuration calculates the backoff duration using exponential backoff with jitter.
func CalculateBackoffDuration(retries int, backoffConfig config.BackoffConfig) time.Duration {
	if retries == 0 {
		return backoffConfig.BaseDelay
	}

	backoff, maxBackoff := float64(backoffConfig.BaseDelay), float64(backoffConfig.MaxDelay)
	for backoff < maxBackoff && retries > 0 {
		// Exponential backoff: baseDelay * multiplier^retries
		backoff *= backoffConfig.Multiplier
		retries--
	}
	...
	// Randomize backoff delays so that if a cluster of requests starts at
	// the same time, they won't operate in lockstep.
	backoff *= 1 + backoffConfig.Jitter*(rand.Float64()*2-1)
	if backoff < 0 {
		backoff = 0
	}
